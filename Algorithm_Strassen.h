...
public:
    void strassenR(Matrix<T>& A, Matrix<T>& B, Matrix<T>& C, int n) //рекурсивна функція, що множить матриці А і В(обов'зково nxn)
    {
        if (n <= 1) {
    ...
    Algorithm_Strassen<T>(Matrix<T>& A, Matrix<T>& B, Matrix<T>& C, int n) //основна функція, спочатку додає нулі до матриць щоб n = 2^k, а потім передає матриці в функцію strassenR
    {
    ...
    void sum(Matrix<T>& A, Matrix<T>& B, Matrix<T>& C, int n) //додавання двох матриць
    {
        int i, j;
        for (i = 0; i < n; i++) {
            for (j = 0; j < n; j++) {
    ...
    void subtract(Matrix<T>& A, Matrix<T>& B, Matrix<T>& C, int n) //віднімання двох матриць
    {
        int i, j;
        for (i = 0; i < n; i++) {
            for (j = 0; j < n; j++) {
