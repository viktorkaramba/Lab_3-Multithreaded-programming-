#ifndef _ALGORITHM_STRASSEN_CPP
#define _ALGORITHM_STRASSEN_CPP
...
void Algorithm_Strassen<T>::strassenR(Matrix<T>& A, Matrix<T>& B, Matrix<T>& C, int n) 
{
    if (n <= 1) {
...
template<typename T>
 Algorithm_Strassen<T>::Algorithm_Strassen(Matrix<T>& A, Matrix<T>& B, Matrix<T>& C, int n) 
{
 ...
 template<typename T>
 void Algorithm_Strassen<T>::sum(Matrix<T>& A, Matrix<T>& B, Matrix<T>& C, int n) {
    int i, j;
    for (i = 0; i < n; i++) {
        for (j = 0; j < n; j++) {
 ...
 template<typename T>
void Algorithm_Strassen<T>::subtract(Matrix<T>& A, Matrix<T>& B, Matrix<T>& C, int n) 
{
    int i, j;
    for (i = 0; i < n; i++) {
        for (j = 0; j < n; j++) {
